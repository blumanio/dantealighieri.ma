...
const MobileNav = () => {
  ...
  return (
    <div className='flex flex-row items-center justify-between'>
      <Link
        href='/'
        className='flex items-center justify-between font-heading text-2xl font-bold text-teal-700'
      >
        ...
        <Sheet open={isOpen} onOpenChange={setIsOpen}>
          ...
          <SheetContent side='right' className='w-[300px] sm:w-[400px]'>
            <div className='flex h-full flex-col'>
              ...
              <nav className='flex flex-grow flex-col items-center justify-start gap-6'>
                {menu.map((item, index) => (
                  <Link
                    href={item.href}
                    key={index}
