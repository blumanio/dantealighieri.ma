...
import {
    ...
    Thermometer,
    Navigation, // For wind or general direction
...
interface WeatherData {
    tempC: number;
    minTempC: number; // Daily min
    maxTempC: number; // Daily max
    description: string;
    iconURL: string;
    isLoading?: boolean;
    error?: boolean;
...
const INITIAL_CARDS_TO_SHOW = 6; // For lazy loading
const CARDS_TO_LOAD_ON_SCROLL = 3; // For lazy loading


// Function to add illustrative annual temps and initial weather state
const initializeCityData = (baseData: Omit<CityData, 'currentWeather' | 'annualMinTempC' | 'annualMaxTempC'>[]): CityData[] => {
    return baseData.map(city => {
        let annualMin = 0, annualMax = 30; // Default illustrative values
        // Assign illustrative annual temps (these are not from an API)
        if (city.id === 'rome') { annualMin = 3; annualMax = 32; }
        else if (city.id === 'milan') { annualMin = -1; annualMax = 30; }
        else if (city.id === 'bologna') { annualMin = 0; annualMax = 31; }
        else if (city.id === 'florence') { annualMin = 2; annualMax = 33; }
        else if (city.id === 'padua') { annualMin = -2; annualMax = 29; }
        else if (city.id === 'naples') { annualMin = 5; annualMax = 30; }

        return {
            ...city,
            annualMinTempC: annualMin,
            annualMaxTempC: annualMax,
                                ...
                                <div key={metric.label}>
                                    <div className="flex items-center justify-between text-sm mb-0.5">
                                        <div className="flex items-center text-[var(--text-secondary)]">
                                            <metric.icon size={16} className="mr-2 text-gray-400" />
                                            {metric.label}
                                        </div>
...
const StudentCityCostInsights: React.FC = () => {
    ...
    const [citiesWithWeatherData, setCitiesWithWeatherData] = useState<CityData[]>(initializeCityData(mockItalianStudentCityData));
    ...
    useEffect(() => {
        if (!API_KEY) {
            console.warn("WeatherAPI.com API key is not set in .env.local (NEXT_PUBLIC_WEATHER_API_KEY). Weather data will not be fetched.");
        ...
        const fetchWeatherForAllCities = async () => {
            const initialCities = initializeCityData(mockItalianStudentCityData); // Get cities with annual temps
            const weatherPromises = initialCities.map(async (city) => {
                try {
                    ...
                    return {
                        ...city, // This already includes annualMinTempC and annualMaxTempC
                        currentWeather: {
            ...
            const citiesWithFetchedWeather = await Promise.all(weatherPromises);
            setCitiesWithWeatherData(citiesWithFetchedWeather);
        };
    ...
    return (
        <div className="bg-[var(--background)] min-h-screen p-4 sm:p-6 md:p-8 text-[var(--text-secondary)]">
            <style jsx global>{`
                ...
                .map-marker-dot {
                    /* Background color applied inline via style attribute */
                }

                .map-marker-city-name {
                ...
                .map-marker-full-detail .custom-map-marker-title {
                    font-weight: bold;
                    font-size: 14px;
                    margin-bottom: 2px;
                    white-space: nowrap;
                ...
                .map-marker-full-detail .custom-map-marker-info {
                    margin: 0;
                    font-size: 11px;
                    white-space: nowrap;
                    overflow: hidden;
