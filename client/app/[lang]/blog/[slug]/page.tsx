...
// Define API_BASE_URL with fallbacks for different environments
const API_BASE_URL = process.env.API_BASE_URL ||
    (process.env.VERCEL_URL ? `https://${process.env.VERCEL_URL}` : 'http://localhost:3000');

// Fetch Single Post Data (Cached) - CORRECTED
const getPost = cache(async (lang: string, slug: string): Promise<BlogPost> => {
    ...
    console.log(`[getPost] Using API_BASE_URL: ${API_BASE_URL}`);

    // Construct the absolute URL
    const targetUrl = `${API_BASE_URL}/api/generated-posts/${slug}?lang=${lang}`;
    console.log(`[getPost] Target URL (absolute): ${targetUrl}`);

    try {
        const res = await fetch(targetUrl, { next: { revalidate: 60 } });

        if (res.status === 404) {
            console.log(`[getPost] Post not found (404) for slug: ${slug}, lang: ${lang}. URL: ${targetUrl}`);
            notFound();
        }
        ...
        } else if (response.slug || response.content) { // Older format or direct data
            postFromApi = response;
        } else {
            console.error(`[getPost] Unexpected API response structure from ${targetUrl}:`, response);
        ...
        const postData: BlogPost = {
            frontmatter: {
                ...
                tags: Array.isArray(frontmatter.tags) ? frontmatter.tags :
                    (postFromApi.tags ? (Array.isArray(postFromApi.tags) ? postFromApi.tags : [postFromApi.tags]) : []),
                coverImage: frontmatter.coverImage || postFromApi.coverImage,
                language: lang // Store the effective language
            },
        ...
        console.error(`[getPost] Error processing post for ${slug}, lang ${lang} (URL: ${targetUrl}):`, error);
        notFound(); // Call notFound for critical errors
        throw error; // Re-throw if needed for upstream error handling or logging
    }
});

// Fetch Adjacent Posts Data (Cached) - Ensure API_BASE_URL is defined
const getAdjacentPosts = cache(async (lang: string, currentSlug: string): Promise<{ prev: any | null; next: any | null }> => {
    ...
    console.log(`[getAdjacentPosts] Using API_BASE_URL: ${API_BASE_URL}`);

    // API_BASE_URL must be defined for this to work. If it's undefined, targetUrl will be malformed.
    if (!API_BASE_URL) {
        console.error("[getAdjacentPosts] API_BASE_URL is not defined. Cannot fetch adjacent posts.");
        return { prev: null, next: null };
    }
    const targetUrl = `${API_BASE_URL}/api/generated-posts/${currentSlug}/adjacent?lang=${lang}`;
    console.log(`[getAdjacentPosts] Target URL (absolute): ${targetUrl}`);
    ...
    try {
        ...
        if (response.success) {
            ...
            console.warn(`[getAdjacentPosts] Unexpected API response structure from ${targetUrl}:`, response);
        }
        ...
        console.error(`[getAdjacentPosts] Error getting adjacent posts from ${targetUrl}:`, error);
        return { prev: null, next: null };
    }
});

// --- Metadata Generation ---
// (generateMetadata will now use the corrected getPost)
export async function generateMetadata(
    { params }: BlogPageProps,
    parent: ResolvingMetadata
): Promise<Metadata> {
    const lang = (await params).lang; // No need for await params if params is not a promise
    const slug = (await params).slug;

    try {
        const post = await getPost(lang, slug); // Uses corrected getPost

        const coverImageUrl = post.frontmatter.coverImage;
        // Ensure your API_BASE_URL is prepended if coverImageUrl is relative
        const absoluteCoverImageUrl = coverImageUrl && !coverImageUrl.startsWith('http')
            ? `${API_BASE_URL}${coverImageUrl.startsWith('/') ? '' : '/'}${coverImageUrl}`
            : coverImageUrl;
        ...
        return {
            title: `${post.frontmatter.title} | Blog`, // Consider your site name
            ...
            openGraph: {
                ...
                images: ogImages,
                locale: post.lang, // Use the post's language
                // siteName: 'Your Site Name', // Add your site name
            },
            alternates: { // Important for SEO with multiple languages
                canonical: pageUrl,
                languages: {
                    // Add other languages your post is available in, e.g.:
                    // 'en-US': `${API_BASE_URL}/en/blog/${slug}`,
                    // 'it-IT': `${API_BASE_URL}/it/blog/${slug}`,
                    [post.lang]: pageUrl // At least list the current one
                },
            },
        };
    } catch (error) {
...
// (BlogPost component will now use the corrected getPost)
export default async function BlogPost({ params }: BlogPageProps) {
    ...
    // If getPost called notFound(), this part might not even render
    // or could be rendering a Next.js error boundary / not-found page.
    // It's good practice to check if 'post' is actually populated if notFound() doesn't fully halt execution.
    if (!post) {
        // This case should ideally be handled by notFound() in getPost,
        // but as a safeguard or if notFound() behavior changes:
        notFound();
    }
    ...
    // ... (rest of your BlogPost component logic remains the same) ...
            ...
            <article className="max-w-4xl mx-auto bg-white p-6 sm:p-8 lg:p-10 rounded-lg shadow-lg">
                ...
                {frontmatter.coverImage && (
                    <div className="mb-8 -mt-6 -mx-6 sm:-mt-8 sm:-mx-8 lg:-mt-10 lg:-mx-10 aspect-[16/9] relative w-[calc(100%+theme(space.12))] sm:w-[calc(100%+theme(space.16))] lg:w-[calc(100%+theme(space.20))] overflow-hidden rounded-t-lg shadow-md">
                        <Image
                            src={frontmatter.coverImage.startsWith('http') ? frontmatter.coverImage : `${API_BASE_URL}${frontmatter.coverImage.startsWith('/') ? '' : '/'}${frontmatter.coverImage}`}
